...
namespace MyEngine
{
	...
	void RendererSoft::drawObject(Object* obj)
	{
		...
		Matrix4 mv = (Renderer::m_pCamera->viewMatrix * (*obj->getMatrix()));
		Matrix4 mvp = Renderer::m_pCamera->projectionMatrix * mv;
	...
	void RendererSoft::rasterize(Vect3f* v, Vect3f* uv, Texture* texture)
	{
		float area;
	...
	void RendererSoft::beginScene()
	{
		zBuff->nulling();
		pixBuff->nulling();
	}

	void RendererSoft::endScene()
	{
		for (int i = 0; i < zBuff->size; i++)
		{
	//		zBuff->data[i] = gammaTable[int(zBuff->data[i] * 255.0f)];
		}

	//	zBuff->saveBMP("zBuff.bmp");
	//	pixBuff->saveBMP("screen.bmp");
	}

	void RendererSoft::setBuffers(int width, int height)
	{
		zBuff = new Buffer<float>(width, height);
		pixBuff = new Buffer<uint32_t>(width, height);
	}
	*/
}